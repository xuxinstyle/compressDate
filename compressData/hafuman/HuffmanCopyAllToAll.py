...
def encode(echo):
    for x in node_dict.keys():
        ...
        if echo == True:  # 输出编码表（用于调试）
            print(x)
            print(ec_dict[x])
def createEncodePath(inputfile ,encodePath):
    split = inputfile.split("\\")
    trueFileName = split[len(split)-1] # name.csv
    nameSplit = trueFileName.split(".")
    return encodePath+nameSplit[0] + ".ys"

def encodefile(inputfile, encodePath):
    print("Starting encode...")
    f = open(inputfile, "rb")
    bytes_width = 2  # 每次读取的字节宽度
    i = 0
    ...
    raw = 0b1
    last = 0
    trueEncodePath = createEncodePath(inputfile ,encodePath)
    name = inputfile.split('.')
    o = open(trueEncodePath, 'wb')
    name = inputfile.split('/')
...
if __name__ == '__main__':
    ...
    if select == '1':
        encodefile(input("请输入要压缩的文件："), encodePath)
    else :
        if select == "3":
            p = input("请输入要压缩的文件夹：")
            for parent, dirname, filenames in os.walk(p):
                for filename in filenames:
                    ...
                    print(filePath)
                    encodefile(filePath, encodePath)

        else:
            decodefile(input("请输入要解压的文件："), decodePath)
